...
class Files {
  ...
  async set(id, { data, url, tg, ...params } = {}) {
    ...
    if (data) {
      const { ext, mime } = await fileType.fromBuffer(data)
      if (!ext || !mime) throw new Error(`Unknown mime from id: ${id}. [ext: ${ext}, mime: ${mime}]`)
      const filePath = path.join(`${parseInt(Date.now() / 1000 / 60 / 60)}`, `${id}.${ext}`)
      await fs.outputFile(path.join(ROOT_DIR, filePath), data)
      newFile.path = `/${filePath}`
      newFile.mime = mime
      newFile.ext = ext
      const size = await imageSize(path.join(ROOT_DIR, filePath))
      if (size.width) newFile.width = size.width
      if (size.height) newFile.height = size.height
    }
