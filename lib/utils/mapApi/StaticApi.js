const qs = require('qs')
const { LARGE, THUMB, ZOOM } = require('./constants')
const PreBaseApi = require('./PreBaseApi')

class StaticApi extends PreBaseApi {
  ...
  placePhotoThumb(photoreference) {
    ...
    return {
      width,
      height,
      url: this.placePhotoLink(photoreference, width, height),
    }
  }
  ...
  static createMaxSize(props, { width, height }) {
    const large = this.relationPhotoSizes(LARGE.WIDTH, LARGE.HEIGHT, width, height)
    const thumb = this.relationPhotoSizes(THUMB.WIDTH, THUMB.HEIGHT, width, height)
